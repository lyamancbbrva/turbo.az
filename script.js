...
const filtersNone = document.getElementById("filters-none");
...
const maxil = document.querySelector("#il-max");
const minil = document.querySelector("#il-min");
const yenielan = document.querySelector(".yenielan");
const mainsearch = document.querySelector(".main-search");
const markaInp = document.querySelector("#marka");
const modelInp = document.querySelector("#model");
const cityInp = document.querySelector("#city");
const valyutaInp = document.querySelector("#valyuta");
const banInp = document.querySelector("#ban");
const preElanlar = document.querySelector(".premium-elanlar");
const qiymetMin = document.querySelector("#qiymet-min")
const qiymetMax = document.querySelector("#qiymet-max")
const kreditBtn = document.querySelector("#kreditBtn")
const barterBtn = document.querySelector("#barterBtn")
const kabinet = document.querySelector('#kabinet')
...
// avtomobilleri ekrana cixaran funksiya
show();
...
// elave inputlari gizleden funksiya

function showHiddenInp() {
    flag = !flag;
    hiddeninp.style.display = flag ? "none" : "flex";
  textA.innerHTML = flag
    ? `Daha çox filtr <i class='fa-solid fa-chevron-down'></i>`
    : `Gizlət <i class='fa-solid fa-chevron-up'></i>`;
}

//optionlarin deyerini seyleyen funksiya
...
getOptions([...new Set(data.cars.map((item) => item.brand).sort())], marka);
getOptions([...new Set(data.cars.map((item) => item.model).sort())], model);
getOptions([...new Set(data.cars.map((item) => item.city).sort())], city);
getOptions([...new Set(data.cars.map((item) => item.currency).sort())], valyuta);
getOptions([...new Set(data.cars.map((item) => item.banType).sort())], ban);
getOptions([...new Set(data.cars.map((item) => item.year).sort((a, b) => b-a))], maxil);
getOptions([...new Set(data.cars.map((item) => item.year).sort((a, b) => b-a))], minil);

// yeni elan bolmesini acan funksiya
function yeniElan() {
    flag = !flag;
    yenielan.style.display = flag ? "none" : "block";
    main.style.display = flag ? "block" : "none";
    mainsearch.style.display = flag ? "block" : "none";
}

//branlari marka hissesinin icine cixaran (mobilde)
...
function changePage(x) {
  ...
  userIcon.style.color = '#B1B5C7FF'

  // red.style.color = 'red'
  if (x == "like") {
    preElanlar.innerHTML = "Seçilmiş Elanlar";
    ...
    if (choosenCars.length == 0) {
      cards.innerHTML += `
        <img src="img/emptyliked.svg" alt="" />
        
       <p style="text-align: center;"> Bəyəndiyiniz elanları ürək işarəsinə klik edərək seçilmişlərə əlavə edin.</p>`;
    } else {
      for (let i = 0; i < choosenCars.length; i++) { 
        
            cards.innerHTML += `
                <div class="card">
                                <div class="card-img">
                                        <img src="${choosenCars[i].images}" />
                                </div>
                                <i class="fa-regular fa-heart" style="color:red; font-weight: bold"  onclick="sil(${choosenCars[i].id})"></i>
                                <div class="card-bottom">
                                    <p class="price">${choosenCars[i].price} ${choosenCars[i].currency}</p>
                                    <p>${choosenCars[i].brand}, ${choosenCars[i].model}</p>
                                    <p>2023, ${choosenCars[i].engine}L, ${choosenCars[i].odometer} ${choosenCars[i].odometerUnit}</p>
                                    <span${choosenCars[i].city},bugün, 13:59</span>
                                </div>
                            </div>
    `
}   
    ...
    homeIcon.style.color = '#ca1016'
    show();
  }else if(x == 'kabinet'){
    kabinet.style.display = 'flex'
    userIcon.style.color = '#ca1016'
...
//filter ve marka hissesini acan funksiya (mobilde)
...
function colorBtn(btn){
  btn.style.border = '1px solid #ca1016'
  btn.style.background = '#ffe6e5'
}
...
for (let i = 0; i < data.cars.length; i++) {
  footerlist.innerHTML += `<a href="">${data.cars[i].model} (${data.cars[i].model.length})</a>`;
}
