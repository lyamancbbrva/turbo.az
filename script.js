...
function like(i, id){
    ...
    let chooseCar =  data.cars.filter(item => id == item.id)
...
function changePage(red, x){
    red.style.color = 'red'
   if (x == like) {
    h3.innerHTML = 'Seçilmişlər'
    cards.innerHTML = ''
    ...
    if (choosenCars.length == 0) {
       ...
       <p style="text-align: center;"> Bəyəndiyiniz elanları ürək işarəsinə klik edərək seçilmişlərə əlavə edin.</p>`
        ...
        choosenCars.map(item =>  cards.innerHTML +=  `
        <div class="card">
                        <div class="card-img">
                                <img src="${item.images}" />
                        </div>
